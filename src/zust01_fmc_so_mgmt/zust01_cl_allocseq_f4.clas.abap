CLASS zust01_cl_allocseq_f4 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_allocseq TYPE TABLE OF zust01_ce_allocseq_f4 WITH DEFAULT KEY.
...
CLASS ZUST01_CL_ALLOCSEQ_F4 IMPLEMENTATION.
  ...
  TRY.
        ...
        DATA et_allocseqs type table of zust01_ce_allocseq_f4.


        DATA(lt_filter_cond) = io_request->get_filter( )->get_as_ranges( ).
        DATA(lt_allocseqs) = get_allocseq_f4( EXPORTING it_filter_cond = lt_filter_cond top = top skip = skip ).
*       OData result/push back to web service

        MOVE-CORRESPONDING lt_allocseqs to et_allocseqs.

        io_response->set_total_number_of_records( lines( et_allocseqs ) ).
        io_response->set_data( et_allocseqs ).
...
* Get allocation sequence F4
    ...
    TYPES: BEGIN OF ty_alloc_seq,
             pasqnc TYPE c LENGTH 40,
...
*               antlf TYPE p DECIMALS 1 LENGTH 1,
    ...
    DATA: lt_pasqnc   TYPE STANDARD TABLE OF ty_alloc_seq WITH NON-UNIQUE KEY pasqnc,
          ls_pasqnc   LIKE LINE OF lt_pasqnc,
...
* Set fields parameter for table read
    ...
    ls_fields-fieldname = 'PRODUCTALLOCATIONSEQUENCE'.
...
* Fill material
    LOOP AT lt_data ASSIGNING <ls_data>.
      ls_pasqnc-pasqnc = <ls_data>(40).
      APPEND ls_pasqnc TO lt_pasqnc.
    ENDLOOP.

    rt_allocseqs = lt_pasqnc.
