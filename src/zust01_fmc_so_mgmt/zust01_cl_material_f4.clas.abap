CLASS zust01_cl_material_f4 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*    TYPES: tt_material TYPE TABLE OF zust01_a_material WITH DEFAULT KEY.
    TYPES: tt_material TYPE TABLE OF zust01_ce_material_F4 WITH DEFAULT KEY.
...
*                it_material           TYPE t_rt_rangeopt
      RETURNING VALUE(rt_materials) TYPE  tt_material
*      RETURNING VALUE(rt_materials) TYPE REF TO ty_tab512
      RAISING
                /iwbep/cx_cp_remote
                /iwbep/cx_gateway
...
CLASS ZUST01_CL_MATERIAL_F4 IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    TRY.
        ...
        DATA et_materials TYPE TABLE OF zust01_ce_material_f4.


        DATA(lt_filter_cond) = io_request->get_filter( )->get_as_ranges( ).
        DATA(lt_materials) = get_material_f4( EXPORTING it_filter_cond = lt_filter_cond top = top skip = skip ).
*       OData result/push back to web service

        MOVE-CORRESPONDING lt_materials TO et_materials.

        io_response->set_total_number_of_records( lines( et_materials ) ).
        io_response->set_data( et_materials ).
...
*               antlf TYPE p DECIMALS 1 LENGTH 1,
    ...
    DATA: lt_matnr   TYPE STANDARD TABLE OF ty_material WITH NON-UNIQUE KEY matnr,
...
* Fill material
    LOOP AT lt_data ASSIGNING <ls_data>.
      ls_matnr-matnr = <ls_data>(40).
      ls_matnr-maktx = <ls_data>+41(40).
      ls_matnr-spras = <ls_data>+40(1).
      APPEND ls_matnr TO lt_matnr.
    ENDLOOP.

    rt_materials = lt_matnr.
