CLASS zust01_cl_soldto_f4 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*    TYPES: tt_soldto TYPE TABLE OF zust01_a_soldto WITH DEFAULT KEY.
    TYPES: tt_soldto TYPE TABLE OF zust01_ce_soldto_F4 WITH DEFAULT KEY.
...
*                it_material           TYPE t_rt_rangeopt
      RETURNING VALUE(rt_soldto) TYPE  tt_soldto
*      RETURNING VALUE(rt_soldto) TYPE REF TO ty_tab512
      RAISING
                /iwbep/cx_cp_remote
                /iwbep/cx_gateway
...
CLASS ZUST01_CL_SOLDTO_F4 IMPLEMENTATION.


METHOD get_soldto_f4.
...
*               antlf TYPE p DECIMALS 1 LENGTH 1,
    ...
    DATA: lt_kunnr   TYPE STANDARD TABLE OF ty_soldto WITH NON-UNIQUE KEY kunnr,
...
*    ENDTRY.
  ...
  METHOD if_rap_query_provider~select.
    TRY.
        ...
        DATA et_soldtos type table of zust01_ce_soldto_f4.


        DATA(lt_filter_cond) = io_request->get_filter( )->get_as_ranges( ).
        DATA(lt_soldtos) = get_soldto_f4( EXPORTING it_filter_cond = lt_filter_cond top = top skip = skip ).
*       OData result/push back to web service

        MOVE-CORRESPONDING lt_soldtos to et_soldtos.

        io_response->set_total_number_of_records( lines( et_soldtos ) ).
        io_response->set_data( et_soldtos ).
