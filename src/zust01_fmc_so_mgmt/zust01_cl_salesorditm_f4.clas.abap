CLASS zust01_cl_salesorditm_f4 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_salesorderitm TYPE TABLE OF zust01_a_salesorderitem WITH DEFAULT KEY.
    METHODS get_sales_orderitm_f4
      IMPORTING
                it_filter_cond        TYPE if_rap_query_filter=>tt_name_range_pairs   OPTIONAL
                top                   TYPE int8 OPTIONAL
                skip                  TYPE int8 OPTIONAL
      RETURNING VALUE(rt_sales_orders_item) TYPE  tt_salesorderitm
      RAISING
                /iwbep/cx_cp_remote
                /iwbep/cx_gateway
...
CLASS zust01_cl_salesorditm_f4 IMPLEMENTATION.
  ...
  TRY.
        ...
        DATA et_salesorders_itm type table of zust01_ce_salesorditm_f4.


        DATA(lt_filter_cond) = io_request->get_filter( )->get_as_ranges( ).
        DATA(lt_salesorders_itm) = get_sales_orderitm_f4( EXPORTING it_filter_cond = lt_filter_cond top = top skip = skip ).
*       OData result/push back to web service

        MOVE-CORRESPONDING lt_salesorders_itm to et_salesorders_itm.

        io_response->set_total_number_of_records( lines( et_salesorders_itm ) ).
        io_response->set_data( et_salesorders_itm ).
  ...
  METHOD get_sales_orderitm_f4.
    ...
    lv_skip = skip.
       ...
       LOOP AT it_filter_cond ASSIGNING FIELD-SYMBOL(<lfs_cond>).
        ...
        lo_response->get_business_data( IMPORTING et_business_data = rt_sales_orders_item ).
