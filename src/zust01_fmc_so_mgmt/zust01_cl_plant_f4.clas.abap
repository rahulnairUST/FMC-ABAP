CLASS zust01_cl_plant_f4 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
*    TYPES: tt_plant TYPE TABLE OF zust01_a_plant WITH DEFAULT KEY.
    TYPES: tt_plant TYPE TABLE OF zust01_ce_plant_F4 WITH DEFAULT KEY.
...
*                it_material           TYPE t_rt_rangeopt
      RETURNING VALUE(rt_plants) TYPE  tt_plant
*      RETURNING VALUE(rt_plants) TYPE REF TO ty_tab512
      RAISING
                /iwbep/cx_cp_remote
                /iwbep/cx_gateway
...
CLASS ZUST01_CL_PLANT_F4 IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    TRY.
        ...
        DATA et_plants TYPE TABLE OF zust01_ce_plant_f4.


        DATA(lt_filter_cond) = io_request->get_filter( )->get_as_ranges( ).
        DATA(lt_plants) = get_plant_f4( EXPORTING it_filter_cond = lt_filter_cond top = top skip = skip ).
*       OData result/push back to web service

        MOVE-CORRESPONDING lt_plants TO et_plants.

        io_response->set_total_number_of_records( lines( et_plants ) ).
        io_response->set_data( et_plants ).
...
*               antlf TYPE p DECIMALS 1 LENGTH 1,
    ...
    DATA: lt_werks   TYPE STANDARD TABLE OF ty_werks WITH NON-UNIQUE KEY werks,
